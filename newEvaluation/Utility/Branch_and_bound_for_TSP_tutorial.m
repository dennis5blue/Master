...
%  ==== STARTUP: Figure anlegen ====
% Mitte des Bildschirms finden
su = get(0,'Units');
set(0,'Units','pixels') % Units auf Pixel stellen
su2 = get(0,'ScreenPixelsPerInch'); % DPI auslesen
if su2 > 96 % Wenn DPI zu groß reduzieren
...
% Panel für Eingabe- und Bedienelemente
...
% Popupmenue zur Auswahl eines Beispiels
...
% Startbutton zum Starten des Algorithmus
...
% Beschriftung des Feldes für der Kostenmatrix
...
% Resetbutton zum Reset des Algorithmus
...
% Suche nach mehreren Branching-Elementen
...
% Details bei der Suche nach Subtouren
...
% Buttongroup für Ungarische Methode
...
% Textfelder für die Ausgaben Panel________________________________________
...
% Initialisierungen%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Anlegen von GUI-Variablen (sind jeder Funktion bekannt)
tableau.nndet = 1; % Default Wert Nächster Nachbar
tableau.umdet = 1; % Default Wert Ungarische Methode
tableau.data = 0; % Variable für Matrix
tableau.kost = 0; % Variable für Kosten
tableau.multbr = 1; % Default Wert multi Branch Elemente suchen
tableau.detail = 0; % Default Wert Details Subtouren
tableau.umtest = 0; % Hilfsvariable für ungarische Methode 1 Mal
guidata(fh,tableau); % Daten aktualisieren
% Callbacks%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Callback für closeRequestFcn
    function closeFig(~,~) % Setzt dpi zurück auf root wert
        dpi = get(fh,'UserData');
...
% Callback für Nächster Nachbar
    function nn_callback(hObject,~)
        ...
        tableau.nndet = get(hObject,'Value'); % Algo mit oder ohne Pausen
        guidata(fh,tableau); % Daten aktualisieren
    end
% Callback für Multbr
    function chbp_callback(hObject,~)
        tableau = guidata(fh); % Daten einlesen
        tableau.multbr = get(hObject,'Value'); % Suche nach mehreren Branchelementen
        guidata(fh,tableau); % Daten aktualisieren
    end
% Callback für Details bei der Orte-Suche
    function chbpDet_callback(hObject,~)
        tableau = guidata(fh); % Daten einlesen
        tableau.detail = get(hObject,'Value'); % Algo mit Detail
        guidata(fh,tableau); % Daten aktualisieren
    end
% Callback für Details bei der Ungarischen Methode
...
% Callback für das Einlesen der Zielfunktion, schreiben der Werte in die
% Achsen
    function editcost_callback(hObject, eventdata)    %#ok
        ...
        [tableau.z tableau.s] = size(tableau.data); % Dimension auslesen
        if tableau.z > 9 || tableau.s > 9 % höchstens 10x10 Matrizen zulassen
            warndlg('Maximum matrix size 9x9 !','Input error!')
            return
        end
        deltaY = (10-tableau.z)*0.03; % Hilfsvariablen zur Positionierung der Matrix
        y = 0.42 + deltaY; %
        yb = 0.375-deltaY;
        ...
        x = 0.01 + deltaX/2;
        posax = [x y xb yb];
        set(haxes1,'Position',posax) % Position setzen
        set(haxes1,'XLim',[0 tableau.s+2],'YLim',[0 tableau.z+2]) % Achsbereich festlegen
        set(haxes2,'Position',[0.01 0.01+deltaY 0.98 0.375-deltaY]) % Position der unteren Achsen festlegen
        set(hp1, 'Visible', 'on') % Sichtbarkeit der Elemente auf an
        set(chbp, 'Enable', 'off') % Bedienelemente abschalten
        ...
        set(startb, 'Enable', 'on') % startbutton enablen
        set(txt0, 'String', 'Cost matrix entered, press start!')
        tableau.start = 1; % Startwert auf 1 setzen
        guidata(fh,tableau);
        colnames = num2cell(1:tableau.z); % Namen für Spalten
        rownames = num2cell(1:tableau.s); % Namen für Zeilen
        tableplot(tableau.data, colnames, rownames); % Matrix in Achsen plotten
    end
% Matrix in Axes schreiben
    function [hca] = tableplot(data, cNames, rNames) % Daten,Spaltennamen, Zeilennamen
        tableau = guidata(fh); % Daten einlesen
        set(fh,'CurrentAxes',haxes1)
        [z s] = size(data);
        tableau.posX = 0.5:s+1.5; % x-Vektor für Koordinaten der Daten
        tableau.posY = z+1.5:-1:0.5; % y-Vektor für Koordinaten der Daten
        celldata = num2cell(data);
        if nargin > 1 % Wenn Zeilen/Spalten namen übergeben würden
            plot(haxes1, [0 s+1],[s+1 s+1], 'LineWidth', 1, 'Color', 'b')
            hold on % Linien zeichen
            plot(haxes1, [1 1],[1 z+2], 'LineWidth', 1, 'Color', 'b')
            set(haxes1, 'XLim', [0 s+3], 'YLim', [0 z+3]) % Achsbereiche bestimmen
            for n = 1:numel(cNames) % Zeilen/Spalten namen als Text plotten
        ...
        for k = 1:z % Daten Plotten
            ...
            end % Hittest aus, damit buttonDownFcn der Figure funktioniert, da
        end     % bei axis off die buttonDownFcn der Achse nicht funktioniert
        hca=get(haxes1,'Children'); % Grafikobjekte auslesen und als Rückgabeargument übergeben
        set(haxes1, 'Visible', 'off')
        guidata(fh,tableau);
    end
% PopupCallback____________________________________________________
    function popup_callback(hObject, ~) % zum Auswählen der bereitgestellten Bsp
        tableau = guidata(fh); % Daten einlesen
...
% Callback für Reset-button_______________________________________________
    function reset_callback(~, ~) % setzt alles zurück - geht nur, wenn
        tableau = guidata(fh); % algo komplett durchgelaufen ist
        tableau.start = 0;
        tableau.kost = 0;
...
% Callback für buttonDownFcn der figure, wird später gesetz
    function axes_resume_callback(~, ~)
        uiresume(fh) % wenn in figure geklickt wird geht es weiter
    end
% Callback für keyPressFcn der figure
    function uiwaitFcn(~, ~)
        uiresume(fh) % wenn in figure geklickt wird, geht es weiter
    end
%Callback für Startbutton_________________________________________________
    function start_callback(~, ~)
        tableau = guidata(fh); % Daten einlesen
        switch tableau.start
            case 1 % am anfang muss tableau.start = 1 sein!
                set(startb, 'Enable', 'off') % startbutton abschalten
                figure(fh)
                set(txt01,'String','Nearest neighbour','Visible','on')
                set(txt0, 'Visible', 'off')
                if tableau.nndet % wird nur ausgeführt, wenn Details ausgewählt
                ...
                A = tableau.data;
                ortAlt = num2str(1);
                Ausgangsort = 1; % es geht immer bei Ort 1 los
                Anfangszeile = Ausgangsort;
                A(:,Anfangszeile) = inf; %ort 1 besucht
                knn = zeros(1,tableau.z); % vektor für Minima
                tableau.plotVek = zeros(1,tableau.z);
                hca=get(haxes1,'Children');
                for n = 1:tableau.z-1
                    [minZeile minIndex] = min(A(Anfangszeile,:)); % Minimum der aktuellen Zeile bestimmen
                    strmin = num2str(minZeile); %  Für Ausgabe
                    ...
                    set(hca(minEl),'Color','r') % Minimum rot färben
                    str1 = num2str(n); % Textausgabe
                    ...
                    ort1 = ortNeu;
                    ortAlt = strInd;
                    A(:,minIndex) = inf; % Besuchten Ort auf unendlich setzen
                    knn(n) = minZeile; % Minimum speichern
                    if n == 1 % Für textausgabe
                ...
                knn(end) = A2(minIndex,Ausgangsort); % Letzter Ort
