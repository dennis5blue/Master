function [totalCost binaryBestSelection] = SAselection_multiSlots (in_numCams,in_testVersion,in_whichSlot,in_searchRange,in_overRange,in_iterLimit)
    %clc;
    %clear;
    addpath('./Utility');
    ...
    % Read files
    vecBits = 8.*dlmread([inputPath 'outFiles/rng' in_searchRange '/slot' in_whichSlot '/indepByte.txt']); % bits
    pos = dlmread([inputPath 'plotTopo/pos.txt']);
    dir = dlmread([inputPath 'plotTopo/dir.txt']);
    matCost = 8.*dlmread([inputPath 'outFiles/rng' in_searchRange '/slot' in_whichSlot '/corrMatrix.txt']);
    ...
    binaryBestSelection = zeros(1,N);
    for i = 1:length(bestSelection)
        binaryBestSelection(bestSelection(i)) = 1;
    end
    
    totalCost = CalSAPayoff( bestSelection, N, pos, bsX, bsY, rho, matCost );
    improveRatio_real = (sum(vecBits) - totalCost)/sum(vecBits);
end