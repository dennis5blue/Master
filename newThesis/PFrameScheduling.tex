...
\end{mythm}
\textbf{\emph{Proof:}}
For camera $i$, if $x_{ii}^* =1$, it means that camera $i$ is an I-frame camera and hence by constraint~\eqref{eq::referenceConstraint} and~\eqref{eq::xConstraintForNonoverheard}, $x_{ik}=0, \forall k \neq i$, which means that the $i^{th}$ row of $\mathbf{X}$ is determined.
...
Moreover, the objective value of Problem~\eqref{eq::formulation} is only affected by $\mathbf{X}$, which means that Problem~\eqref{eq::formulation} can be transform from an ``optimization problem'' into a ``decision problem'' after $\mathbf{X}$ is determined by the \emph{I-frame selection sub-problem}.
For this reason, we formulate the \emph{P-frame scheduling sub-problem} as a decision problem as follows:
\begin{align}
	&\text{If} \quad \mathbf{X} \quad \text{is feasible}, & \nonumber \\
\text{subject to} & & \nonumber \\
...
\end{align}
The only question we need to solve in Problem~\eqref{eq::PFrameSchedulingSubProb} is that whether $\mathbf{X}$ obtained by \emph{I-frame selection sub-problem} is a feasible solution of the original formulation.
\begin{mythm}
The solution of Problem~\eqref{eq::PFrameSchedulingSubProb} is always true if $\mathbf{X}$ is created by our proposed I-frame selection algorithm together with theorem~\ref{theorem::determineX}.
\label{theorem::IFrameSolalwaysFeasible}
\end{mythm}
\textbf{\emph{Proof:}}
Obviously\hfill$\mathsmaller{\mathsmaller{\blacksquare}}$
